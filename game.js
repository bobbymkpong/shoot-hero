...
$.renderInterface = function() {
	...
	==============================================================================*/
		...
		if( $.instructionTick < $.instructionTickMax ){
			...
			$.ctxmg.fillStyle = 'rgba(255,0,0,' + alpha + ')';
			$.ctxmg.fill();
...
$.setState = function( state ) {
	...
	if( state == 'credits' ) {
		...
		var js13kButton = new $.Button( {
			...
			action: function() {
				location.href = 'file:///C:/Users/Aidee/Downloads/blog/home.html';
				$.mouse.down = 0;
			}
		} );
...
$.setupStates = function() {
	$.states['menu'] = function() {
		...
		var bottomInfo = $.text( {
			...
			x: $.cw / 2,
			y: $.ch - 172,
			text: 'CREATED BY UBOKUTOM EKEMINI MKPONG',
			hspacing: 1,
			vspacing: 1,
			halign: 'center',
			valign: 'bottom',
	...
	$.states['credits'] = function() {
		...
		var creditKeys = $.text( {
			...
			text: 'DEVELOPED BY UBOKUTOM MKPONG \nSUPPORTED BY DAVID EGWENU \n AND WISDOM ENANG ',
			hspacing: 1,
			vspacing: 17,
			halign: 'right',
			valign: 'top',
